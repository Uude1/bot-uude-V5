...
exports.default = new Command_1.Command({
        ...
        },
        {
            ...
            options: [{
                ...
                },
                {
                    ...
                    type: discord_js_1.ApplicationCommandOptionType.Channel,
                    channelTypes: [discord_js_1.ChannelType.GuildCategory],
                    required: true
                },
                {
        ...
        switch (Subcommand) {
            ...
            case "ticket": {
                ...
                if (options.getSubcommand() === "ticket") {
                    ...
                    await guildData.save();
                    if (channel instanceof discord_js_1.TextChannel) {
                        const embed_ticket = new discord_js_1.EmbedBuilder()
                        ...
                        const selec_ticket = new discord_js_1.ActionRowBuilder({
                            components: [
            ...
            case "status": {
                ...
                server.getPlayers().then(async (players) => {
                    if (canal_status instanceof discord_js_1.TextChannel) {
                        let embed = new discord_js_1.EmbedBuilder()
                            .setAuthor({ name: `${interaction.guild.name}` })
                            .setThumbnail(interaction.guild.iconURL())
                            .setColor(__1.config.colors.corbot)
                            .addFields({ name: '> __Players:__', value: `\`\`\`\n🎮 ${players}\n\`\`\``, inline: true }, { name: '> __Status:__', value: '```\n🟢 Online\n```', inline: true }, { name: '> __CONECTAR-SE PELO F8 NO FIVEM COM O__\n> __CONNECT DA CIDADE:__', value: `\`\`\`\n ${interaction.options.getString('connect')}\n\`\`\`` });
                        if (imagem) {
                        ...
                        let offline_embed = new discord_js_1.EmbedBuilder()
                            ...
                            .setColor(__1.config.colors.corbot)
                            .setThumbnail(interaction.guild.iconURL())
                            .addFields({ name: '> __Players:__', value: `\`\`\`\n🎮 Sem informação\n\`\`\``, inline: true }, { name: '> __Status:__', value: '```\n🔴 desligado!\n```', inline: true }, { name: '> __CONECTER-SE PELO F8 NO FIVEM COM O__\n> __CONNECT DA CIDADE:__', value: `\`\`\`\n ${interaction.options.getString('connect')}\n\`\`\`` });
                        if (imagem) {
                            offline_embed.setImage(`${imagem}`);
                        }
                        let conectar = new discord_js_1.ButtonBuilder({
                            url: `${link}`,
                            style: discord_js_1.ButtonStyle.Link,
                            label: "Conectar",
                            emoji: "1003002157106200666",
                        });
                        let lojavip = new discord_js_1.ButtonBuilder({
                            url: `${vip}`,
                            style: discord_js_1.ButtonStyle.Link,
                            label: "Loja Vip",
                            emoji: "1098691501782994996"
                        });
                        let row = new discord_js_1.ActionRowBuilder({ components: [conectar, lojavip] });
    ...
    selects: new discord_js_1.Collection([
        ["select_ticket", async (selectInteraction) => {
                ...
                let text;
                if (selectInteraction.values[0] === 'categorydoacoes')
                    text = 'doações';
                if (selectInteraction.values[0] === 'categorysuporte')
                    text = 'suporte';
                if (selectInteraction.values[0] === 'categorydenuncias')
                    text = 'denuncias';
                ...
                if (!selectInteraction.guild?.channels.cache.get(categoria))
                    categoria = null;
                const ticket = selectInteraction.guild?.channels.cache.find((ticket) => ticket.type === discord_js_1.ChannelType.GuildText && ticket.topic === selectInteraction.user.id);
                if (ticket) {
                    let embed = new discord_js_1.EmbedBuilder()
                    ...
                    setTimeout(() => {
                        selectInteraction.guild?.channels.create({
                            ...
                            .then((channel) => {
                            const embed_compra = new discord_js_1.EmbedBuilder()
                                .setColor(__1.config.colors.corbot)
                            ...
                            const choseticket = new discord_js_1.ButtonBuilder({
                                customId: "fechar_ticketember",
                                style: discord_js_1.ButtonStyle.Danger,
                                label: "Fechar Ticket",
                                emoji: "❌"
                            });
                            const addmember = new discord_js_1.ButtonBuilder({
                                customId: "addmember",
                                style: discord_js_1.ButtonStyle.Secondary,
                                label: "Adicionar Membro",
                                emoji: "<:optimus_adicionar:1124036932406616185>"
                            });
                            const remember = new discord_js_1.ButtonBuilder({
                                customId: "remember",
                                style: discord_js_1.ButtonStyle.Secondary,
                            ...
                            const row = new discord_js_1.ActionRowBuilder({ components: [choseticket, addmember, remember] });
    ...
    buttons: new discord_js_1.Collection([
        ["fechar_ticketember", async (interactionTicket) => {
                ...
                if (interactionTicket.channel.type !== discord_js_1.ChannelType.GuildText) {
                ...
                const file = await (0, discord_html_transcripts_1.createTranscript)(interactionTicket.channel, {
                ...
                const embed2 = new discord_js_1.EmbedBuilder()
                    ...
                    .addFields({ name: "Usuário que abriu o ticket: ", value: `${user.user.tag}`, inline: false }, { name: "Usuário que fechou o ticket:", value: `${interactionTicket.user.tag}`, inline: false }, { name: "Horário de abertura do ticket:", value: `<t:${~~(interactionTicket.createdAt.getTime() / 1000)}:F>`, inline: false }, { name: "Canal do ticket:", value: `${interactionTicket.channel.name}`, inline: false });
                setTimeout(() => {
                    user.send({ embeds: [embed2], files: [file] });
                    if (logs instanceof discord_js_1.TextChannel && logs.guild.id === interactionTicket.guild.id) {
        ...
        ["addmember", async (interactionaddmember) => {
                if (!interactionaddmember.inCachedGuild())
                    return;
                const modal = new discord_js_1.ModalBuilder({ customId: "addmember", title: "Adicione um membro ao ticket" });
                const input1 = new discord_js_1.ActionRowBuilder({
                    components: [
        ...
        ["remember", async (interactionaddmember) => {
                if (!interactionaddmember.inCachedGuild())
                    return;
                const modal = new discord_js_1.ModalBuilder({ customId: "addmember", title: "Remover um membro ao ticket" });
                const input1 = new discord_js_1.ActionRowBuilder({
                    components: [
                        new discord_js_1.TextInputBuilder({
        ...
        ["reportbug", async (reportbug) => {
                if (!reportbug.inCachedGuild())
                    return;
                const modal = new discord_js_1.ModalBuilder({ customId: "reportabug", title: "REPORTAR BUG/ERRO" });
                ...
                const imput3 = new discord_js_1.ActionRowBuilder({
                    components: [
                        new discord_js_1.TextInputBuilder({
                            customId: "bugimagem1",
                ...
                const imput4 = new discord_js_1.ActionRowBuilder({
                    components: [
                        new discord_js_1.TextInputBuilder({
                            customId: "bugimagem2",
                ...
                let embed = new discord_js_1.EmbedBuilder()
                    ...
                    .addFields({ name: 'Bug', value: bug }, { name: 'Imagem', value: image || 'Nenhuma imagem fornecida' });
